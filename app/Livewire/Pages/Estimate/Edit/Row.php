        ...
        elseif ($this->row->position == 0) {
            ...
            elseif ($this->row->section->position > 0) {
                $this->row->section->rows()
                    ...
                    ->decrement('position');
                $this->row->estimate_section_id = $this->row->section->estimate->sections()->where('position', $this->row->section->position - 1)->first()->id;
                $this->row->position = $this->row->section->estimate->sections()->where('position', $this->row->section->position - 1)->first()->rows()->count();
                $this->row->save();
                $this->dispatch('sectionRerender');
            }
            ...
            elseif ($this->row->section->position == 0 && $this->row->section->estimate->sections()->count() > 1) {
                $this->row->section->rows()
                    ->decrement('position');
                $this->row->estimate_section_id = $this->row->section->estimate->sections()->reorder('position', 'desc')->first()->id;
                $this->row->position = $this->row->section->estimate->sections()->reorder('position', 'desc')->first()->rows()->count();
    ...
    public function rowDown()
    {
        ...
        elseif ($this->row->position == $this->row->section->rows()->count() - 1) {
            ...
            elseif ($this->row->section->estimate->sections()->count() > 1 && $this->row->section->position < $this->row->section->estimate->sections()->count() - 1) {
                $this->row->section->estimate->sections()->where('position', $this->row->section->position + 1)->first()->rows()
                    ->increment('position');
                $this->row->estimate_section_id = $this->row->section->estimate->sections()->where('position', $this->row->section->position + 1)->first()->id;
                $this->row->position = 0;
                $this->row->save();
            ...
            elseif ($this->row->section->estimate->sections()->count() > 1 && $this->row->section->position == $this->row->section->estimate->sections()->count() - 1) {
                $this->row->section->where('position', '==', 0)->first()->rows()
                    ->increment('position');
