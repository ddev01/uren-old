<div {{ $attributes->merge(['class' => 'h-full']) }}>
    ...
    <div class="relative h-full">
        ...
        @endif
        <textarea class="h-full min-h-[40px] w-full estimate-textarea rounded-sm bg-gray-800 px-4 py-2 focus:outline-none resize-y overflow-auto" id="{{ $name }}" name="{{ $name }}" {{ $attributes }} x-data="{
            resize: () => {
