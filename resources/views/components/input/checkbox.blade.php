<div class="{{ $attributes['class'] }}">
	...
	<div class="relative flex h-full items-end">
		...
		<div class="relative h-fit w-fit flexc">
			<input class="no-spinners aspect-square h-full min-h-[40px] rounded-sm bg-gray-800 px-4 py-2 focus:outline-none" id="{{ $name }}" name="{{ $name }}" type="checkbox" style="appearance: none" {{ $attributes }}>
		</div>
		@if ($suffix ?? false)
